...
//EJERCICIO EXAMEN (metodoGrafo) ABAJO DEL TODO
...
public abstract class Grafo {
    ...
    public boolean metodoGrafo(int n, int p) {
        UFSet ufset = new ForestUFSet(numVertices());
        ...
        for (int i = 0; i < numVertices(); i++) {
            ...
            for (l.inicio(); !l.esFin(); l.siguiente()) {
                Adyacente a = l.recuperar();
                if (a.getPeso() >= p) {
                    int findI = ufset.find(i);
                    int findDestino = ufset.find(a.destino);
                    if (findI != findDestino) {
                        ufset.union(findI, findDestino);
                        vertices--;
                        if (vertices < n) return false;
                    }
                }
            }
        }
