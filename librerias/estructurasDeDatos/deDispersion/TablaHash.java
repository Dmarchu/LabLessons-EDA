...
public class TablaHash<C, V> implements Map<C, V> {
     ...
     */
    private static boolean REHASHING = true; // en Parte 1 vale false;
    ...
    protected final void rehashing() {
        //Primero calculo la nueva capacidad 
        int nuevaCapacidad = siguientePrimo(2*elArray.length);
        //Tengo que guardar el array anterior con todas sus entradas
        ListaConPI<EntradaHash<C, V>>[] elArrayAnterior = this.elArray;
        //Tengo que inicializar un nuevo elArray con una nueva capacidad
        ...
        //Reinicio la talla
        this.talla = 0;
        //Ahora me toca insertar las entradas en la tabla
        for(int i = 0; i < elArrayAnterior.length; i++){
            ListaConPI<EntradaHash<C,V>> cubeta = elArrayAnterior[i];
    ...
    public final double desviacionTipica() {
        //Primero obtengo la longitud media de las cubetas
        double lMedia = (double) talla / (double) elArray.length; //recuerda hacer casting!!!
        double sum = 0.0;
        //Ahora calculamos el numerador (sumatorio) de la ecuación
        for(int i = 0; i < elArray.length; i++){
            sum += Math.pow(elArray[i].talla() - lMedia, 2);
        }
    ...
    public final double costeMLocalizar() {
        if(talla == 0) return 0;
        //declaramos una variable para guardar las colisiones
        double colisiones = 0.0;
        //hacemos el bucle para ir almacenando en colisiones por cubeta
        for(int i = 0; i < elArray.length; i++){
            //declaro una variable para que sea mas facil entenderlo despues
            double n = elArray[i].talla();
            //voy sumando
            colisiones += n * ((n-1)/2);
        }
        return colisiones/talla;
    }
