...
public class GrafoRedMunicipios {
    ...
    public GrafoRedMunicipios(String nomFich) {
        ...
        String nomFichMunicipios = DIR_FICHEROS + nomFich + "P1_municipios.txt";
        try {         
    ...
    private void cargarAristas(String nomFich) {  
        String nomFichCostes = DIR_FICHEROS  + nomFich + "P1_costes.txt";
        try {
    ...
    public double crearAdyKruskal() {      
        ...
        for(int i = 0; i < aristas.length; i++){
            ...
            Municipio muOrigen = verticesAMunicipios.recuperar(origen);
            Municipio muDestino = verticesAMunicipios.recuperar(destino);
            if(adyKruskal.recuperar(muOrigen) == null){
                adyKruskal.insertar(muOrigen, new LEGListaConPI<Municipio>());
            }
            if(adyKruskal.recuperar(muDestino) == null){
                adyKruskal.insertar(muDestino, new LEGListaConPI<Municipio>());
            }
            adyKruskal.recuperar(muOrigen).insertar(muDestino);
            adyKruskal.recuperar(muDestino).insertar(muOrigen);
            coste += arista.getPeso();
        }
